...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        while True:
            ...
            if cmd == "GLOBAL":
                # recv global
                size, t_recv = recv_file(sock, "global.pt")
                try:
                    sd = torch.load("global.pt", map_location=DEVICE, weights_only=True)
                except TypeError:
                    sd = torch.load("global.pt", map_location=DEVICE)
                ...
                # send local
                out_path = f"local_c{CID}_r{msg['round']}.pt"
