...
# ---------------- LOAD CONFIG ----------------
BP = getattr(config, "BACKPROP", {})
SEED         = BP.get("SEED", getattr(config, "SEED", 42))
BATCH_SIZE   = BP.get("BATCH_SIZE", getattr(config, "BATCH_SIZE", 32))
LR           = BP.get("LEARNING_RATE", getattr(config, "LEARNING_RATE", 5e-5))
WEIGHT_DECAY = BP.get("WEIGHT_DECAY", getattr(config, "WEIGHT_DECAY", 0.0))
